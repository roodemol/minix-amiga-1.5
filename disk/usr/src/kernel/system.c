...
register message *m;
{
  ...
  case 3:			/* return value from process table */
	if ((int) ADDR >= sizeof(struct proc)) return(EIO);
  ...
  case 6:			/* set value in process table */
	i = (unsigned) ADDR;
	if (i + sizeof(reg_t) >= sizeof(struct stackframe_s)) return(EIO);
#if (CHIP == INTEL)
...
#include <minix/amtransfer.h>
  ...
  static struct transferdata *transdat;
  static int chunk_num = 1;
  long address, size, used_start;

  transdat = *(struct transferdata **)0x0000;

  used_start = *(long *)0x0004;

  if (chunk_num >= transdat->numintranslist) {
	m_ptr->COUNT = 0;
	m_ptr->POSITION = 0;
  } else {
	address = transdat->transmemlist[chunk_num];
	size = 0;
	do {				/* compact adjacent chunks */
		size += MEMCHUNKSZ;
		++chunk_num;
	} while ((chunk_num < transdat->numintranslist) &&
		(address + size == transdat->transmemlist[chunk_num]));

	if (used_start >= address && CHIPMEM == address + size)
		size -= (CHIPMEM - used_start);
	m_ptr->POSITION = address >> CLICK_SHIFT;
	m_ptr->COUNT = size >> CLICK_SHIFT;
  }
  return(OK);
}
