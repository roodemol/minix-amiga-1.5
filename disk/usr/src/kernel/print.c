#if (CHIP == M68000)
/* This file contains the printer driver for the Atari ST and Amiga
...
#define PRINTER_BUSY (*PRAB & PR_BUSY)
#endif
#if (MACHINE == ATARI)
#include "staddr.h"
#include "stsound.h"
#include "stmfp.h"
#define PRINTER_BUSY (MFP->mf_gpip & IO_PBSY)
#endif
...
PUBLIC void printer_task()
{
  ...
  *DDRBA=0xff;          /* centronics port in output mode (PRBA) */
  *DDRAB=0xc0;          /* printer bits in input mode (PRAB) */
...
register message *mp;
{
  ...
  if (r == OK) {
	busy = TRUE;
	prt_ack_int();	/* print first character */
 ...
 *				prt_ack_int  				     *
 *===========================================================================*/
PUBLIC void prt_ack_int()
{
/* This is the Parallel Interface Adapter interrupt handler.
 * When a character has been printed, an
 * interrupt occurs, and the assembly code calls prt_ack_int().
 */

#if (MACHINE == AMIGA)
  disable_ciaa_int(ICRF);
#endif
  if (busy == FALSE) {
