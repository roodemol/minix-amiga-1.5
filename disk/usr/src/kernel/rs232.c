/* rs232 driver for the PC, ST and AMIGA */
...
#endif
#if (MACHINE == AMIGA)
#define SIAIN(c)	(c) = getc_rs232()
#ifdef DEBUG
#define SIAOUT(c)	{\
    lastchar = (c);\
    printf("sending 0x%04x\n", ctab[lastchar]);\
    *SERDAT=ctab[lastchar];\
}
#else
#define SIAOUT(c)	{\
    lastchar = (c);\
...
#if (MACHINE == AMIGA)
#define CTABSZ 256
PRIVATE u_short ctab[CTABSZ];
PRIVATE u_short rec_mask; /* receive mask to mask out stopbits */
PRIVATE u_short dat_mask; /* receive mask to get databits only */
#endif
...
#endif
#if (MACHINE == AMIGA)
  *INTENA = WSET|RBF|TBE;	/* enable receive buffer full and transmit
...
int data_bits;			/* 5, 6, 7, or 8 */
{
  ...
  if (in_baud != out_baud) {
    printf("input baudrate and output baudrate should be equal,\n");
    if (in_baud>out_baud)
        out_baud=in_baud;
    else
        in_baud=out_baud;
    printf("setting both to %d baud.\n", out_baud);
  }
  init_ctab(stop_bits, data_bits, parity, out_baud);
  tty_struct[line].tty_speed = ((out_baud/100) << 8) | (in_baud/100);
...
/* the biggest difference between the amiga and the pc and st
 * is that they can do parity control in hardware, amiga's cannot
 */

/* #define CHECKREADPARITY  */

/* if defined, parity is checked on incoming data */

PRIVATE stop_bits, data_bits, parity;

init_ctab(stopbits, databits, par, baudrate)
int stopbits;
int databits;
int par;
unsigned int baudrate;
{
    int i, firststop, pbit, bitpos, j;
    ...
    if (baudrate<=0)
        baudrate=1;
    for (i=0; i<CTABSZ; ++i) {
        ctab[i]=i & ((1<<data_bits)-1);
        if (parity==NONE)
            firststop=data_bits;
        else {
            /* compute the parity bit */
            firststop=data_bits+1; /* keep space for parity bit */
            for (pbit=0, bitpos=0; bitpos<data_bits; ++bitpos)
                if (i & (1<<bitpos))
                    pbit^=1;
            if (parity==ODD)
                pbit^=1;
        ...
        /* set the stopbits */
        for (j=stop_bits, bitpos=firststop; j>0; --j, ++bitpos)
            ctab[i]|=1<<bitpos;
    }
    dat_mask=(1<<data_bits)-1;
    rec_mask=(1<<(data_bits+stop_bits+(parity==NONE ? 0 : 1)))-1;
    *SERPER= (0x7fff & (u_short)(3579098L/(unsigned long)baudrate -1))
     | (firststop<=8 ? 0 : 0x8000);
}


int getc_rs232()
{
    register u_short c, d;

    *INTREQ=WCLR|RBF;
    c= *SERDATR & rec_mask;
    d=c & dat_mask;
#ifdef CHECKREADPARITY
    if ((ctab[d] & rec_mask) != c)
        printf("serial driver parity error: received 0x%04x, should be 0x%04x\n", 
         c, ctab[d]);
#endif
    return d;
}
