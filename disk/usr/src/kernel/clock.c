...
{
/* This routine is called on clock ticks when a lot of work needs to be done */
...
PRIVATE void init_clock()
{
/* Initalize timer B of cia-A to e.g. 60 Hz. */
  ...
  freq = transdat->args['t'-'a'];	/* depends on PAL/NTSC */
  count = freq / 10 / HZ;
  *TBLOA = count & 0xFF;
  *TBHIA = (count >> 8) & 0xFF;

  *CRBA = 0x01;					/* start timer */
  enable_ciaa_int(ICRTB);
  enable_int(PORTS);			/* enable CIAA-interrupts */
}
